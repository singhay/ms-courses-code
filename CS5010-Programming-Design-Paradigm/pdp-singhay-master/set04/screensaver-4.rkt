...
#reader(lib "htdp-beginner-abbr-reader.ss" "lang")((modname screensaver-4) (read-case-sensitive #t) (teachpacks ()) (htdp-settings #(#t constructor repeating-decimal #f #t none #f () #f)))
;; screensaver-4.rkt : Second Question of Problem Set 04. 
;; Description:
...
;; Pen Down by selecting the rectangle via mouse and
;; pressing "d" button on keyboard.
;; Pen Up by selecting the rectangle via mouse and
;; pressing "u" button on keyboard.
...
(begin-for-test  
  ...
  (check-equal?
   (rect-after-key-event INITIAL-RECT-1 PEN-DOWN-KEY-EVENT)
  ...
  (check-equal?
   ...
   (toggle-pen-down INITIAL-RECT-1 false)
...
(define (new-rectangle x y vx vy)
 (make-rect x y vx vy false ZERO ZERO false (list empty)))
;; TESTS
...
(define RECT-X-ORIG-Y-ORIG-BTNDWN
  (make-rect 40 X-36 -23 INITIAL-RECT-2-VY true -30 6 false (list empty)))
(begin-for-test
  (check-equal?
  ...
  (check-equal?
   ...
   "Should return a same rectangle")
  (check-equal?
   (rect-after-button-down RECT-X-ORIG-Y-ORIG 170 (/ CANVAS-WIDTH 2))
   RECT-X-ORIG-Y-ORIG
...
(define RECT-X-ORIG-Y-ORIG-BTNDRAG
  (make-rect 40 X-36 -23 INITIAL-RECT-2-VY true ZERO ZERO false (list empty)))
(define RECT-X-ORIG-Y-ORIG-BTNDRAGED
  (make-rect 170 X-ORIGIN -23 INITIAL-RECT-2-VY true ZERO ZERO false (list empty)))
(begin-for-test
  (check-equal? 
   (rect-after-drag RECT-X-ORIG-Y-ORIG-BTNDRAG 170 X-ORIGIN)
   RECT-X-ORIG-Y-ORIG-BTNDRAGED
   "Should return a new rectangle
    with rect-selected? set to true")
  (check-equal?
   (rect-after-drag RECT-X-ORIG-Y-ORIG 170 X-ORIGIN)
   RECT-X-ORIG-Y-ORIG
...
(begin-for-test
  (check-equal?
   (in-rect? RECT-X-ORIG-Y-ORIG 170 X-ORIGIN)
   false
...
;; EXAMPLES: see tests below
;; STRATEGY: Use template for ListOfPoints on LOP
...
;; EXAMPLES: see tests below
;; STRATEGY: Divide into case on whether point is empty or not?
(define (place-point point scene)
  (cond 
    [(empty? point) scene]
    [else
      (place-point-scene point scene)]))  
;; TESTS
(begin-for-test
  (check-equal?
   (place-point POINT-35-45 EMPTY-CANVAS)
   (place-point-scene POINT-35-45 EMPTY-CANVAS)
   "Should return a scene where a new point marked on an EMPTY-CANVAS
    with a PEN-DOT at (35,45) inherent in the point data type.")
  (check-equal?
   (place-point empty EMPTY-CANVAS)
   EMPTY-CANVAS
   "Should return an EMPTY-CANVAS since input point is empty."))

;; place-point-scene : Point Scene -> Scene
;; GIVEN   : a Point(point) and a Scene(scene)
;; RETURNS : a Scene with the GIVEN Point or a PEN-DOT
;;           placed on scene using point coordinates 
;;           inherent in Point on the the given scene.
;; EXAMPLES: see test below
;; STRATEGY: Use template for Rectangle on rect
(define (place-point-scene point scene)
  (place-image
   PEN-DOT
...
(begin-for-test
  ...
  (place-image
       ...
       X-35 Y-45
       (place-points (list empty) EMPTY-CANVAS))
...
(define UNTOUCHED-LIMITS
  (make-rect (/ CANVAS-WIDTH 2) (/ CANVAS-WIDTH 2) INITIAL-RECT-2-VX INITIAL-RECT-2-VY false ZERO ZERO false (list empty)))
(define UNTOUCHED-LIMITS-MOVED
  (make-rect (+ (/ CANVAS-WIDTH 2) INITIAL-RECT-2-VX) (- (/ CANVAS-WIDTH 2) (- INITIAL-RECT-2-VY)) INITIAL-RECT-2-VX INITIAL-RECT-2-VY false ZERO ZERO false (list empty)))
(define TOUCHED-X-ORIGIN
  (make-rect ZERO (/ CANVAS-WIDTH 2) INITIAL-RECT-2-VX INITIAL-RECT-2-VY false ZERO ZERO false (list empty)))
(define TOUCHED-X-ORIGIN-INV
  (make-rect X-ORIGIN (- (/ CANVAS-WIDTH 2) (- INITIAL-RECT-2-VY)) VX-NEG-23 INITIAL-RECT-2-VY false  ZERO ZERO false (list empty)))
(define TOUCHED-Y-ORIGIN
  (make-rect (/ CANVAS-WIDTH 2) INITIAL-RECT-1-VY INITIAL-RECT-2-VX INITIAL-RECT-2-VY false  ZERO ZERO false (list empty)))
(define TOUCHED-Y-ORIGIN-INV
  (make-rect (+ (/ CANVAS-WIDTH 2) INITIAL-RECT-2-VX) Y-ORIGIN INITIAL-RECT-2-VX VY-14 false  ZERO ZERO false (list empty))) 
(define TOUCHED-X-BOUNDARY
  (make-rect (- CANVAS-WIDTH HALF-RECTANGLE-WIDTH 1) (/ CANVAS-WIDTH 2) INITIAL-RECT-2-VX INITIAL-RECT-2-VY false  ZERO ZERO false (list empty)))
(define TOUCHED-X-BOUNDARY-INV
  (make-rect X-BOUNDARY (- (/ CANVAS-WIDTH 2) (- INITIAL-RECT-2-VY)) VX-NEG-23 INITIAL-RECT-2-VY false  ZERO ZERO false (list empty)))
(define TOUCHED-Y-BOUNDARY
  (make-rect (/ CANVAS-WIDTH 2) (* CANVAS-WIDTH 2) INITIAL-RECT-2-VX INITIAL-RECT-2-VY false  ZERO ZERO false (list empty)))
(define TOUCHED-Y-BOUNDARY-INV
  (make-rect (+ (/ CANVAS-WIDTH 2) INITIAL-RECT-2-VX) Y-BOUNDARY INITIAL-RECT-2-VX VY-14 false  ZERO ZERO false (list empty)))

(define RECT-X-ORIG-Y-ORIG
  (make-rect X-40 X-36 VX-NEG-23 INITIAL-RECT-2-VY false  ZERO ZERO false (list empty)))
(define RECT-X-ORIG-Y-ORIG-INV
  (make-rect X-ORIGIN Y-ORIGIN INITIAL-RECT-2-VX VY-14 false  ZERO ZERO false (list empty)))
(define RECT-X-ORIG-Y-ORIG-SELECTED
  (make-rect X-40 X-36 VX-NEG-23 INITIAL-RECT-2-VY true  ZERO ZERO false (list empty)))
(define RECT-X-ORIG-Y-ORIG-SELECTED-INV
  (make-rect X-40 X-36 VX-NEG-23 INITIAL-RECT-2-VY true ZERO ZERO false (list empty)))

(define RECT-X-ORIG-Y-BOUND
  (make-rect X-40 (- CANVAS-HEIGHT HALF-RECTANGLE-HEIGHT 1) VX-NEG-23 VY-14 false  ZERO ZERO false (list empty)))
(define RECT-X-ORIG-Y-BOUND-INV
  (make-rect X-ORIGIN Y-BOUNDARY INITIAL-RECT-2-VX INITIAL-RECT-2-VY false  ZERO ZERO false (list empty)))

(define RECT-X-BOUND-Y-ORIG
  (make-rect (- CANVAS-WIDTH HALF-RECTANGLE-WIDTH 1) X-36 X-40 INITIAL-RECT-2-VY false  ZERO ZERO false (list empty)))
(define RECT-X-BOUND-Y-ORIG-INV
  (make-rect X-BOUNDARY Y-ORIGIN (- X-40) VY-14 false  ZERO ZERO false (list empty)))

(define RECT-X-BOUND-Y-BOUND
  (make-rect (- CANVAS-WIDTH HALF-RECTANGLE-WIDTH 1) (- CANVAS-HEIGHT HALF-RECTANGLE-HEIGHT 1) X-40 VY-14 false  ZERO ZERO false (list empty)))
(define RECT-X-BOUND-Y-BOUND-INV
  (make-rect X-BOUNDARY Y-BOUNDARY (- X-40) INITIAL-RECT-2-VY false  ZERO ZERO false (list empty)))
(begin-for-test
  (check-equal?
...
(define (velocity-inverted rect)
  (make-rect
   ...
   (if (rect-pen-down? rect)
       (append
        (list (make-posn (rect-x rect) (rect-y rect)))
        (rect-points rect))
       (rect-points rect))))
;; TESTS
(begin-for-test
...
;; RETURNS : Whether or not rectangle touches in next tick
